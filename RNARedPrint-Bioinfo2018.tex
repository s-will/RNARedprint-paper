...
the computational cost in check, we exploited the close correlation of
...
for any structure $R$ and sequence $S$. We learned the
$(\gamma,\delta)$ parameters from a set of sequences generated with
homogenous weights $w=e^{\beta}$, tuning only the \GCb\% to a
...
Fig.~\ref{fig:energydist} illustrates the described
strategy. \todo{Update Figure 2A}Fig.~\ref{fig:energydist}A
illustrates our capability to target Turner energies based on the
stacking energy model. It shows the relative distance between
$\EnergyTurner$ and $\EnergyStacking$, observed for instances of the
\texttt{Modena} benchmark. For each structure, $n=1\,000$ sequences
were sampled, while targeting a ${\sf GC}\%$ of $50\%$, and reported
the difference between the expected and sampled energies for both the
Turner model and its approximation using a stacking model.  For an
example instance of the \texttt{Modena} benchmark \texttt{3str},
Fig.~\ref{fig:energydist}B shows the Turner energy distributions of
the single structures as they result from sampling with different weight
parameters. The figure illustrates how our multidimensionnal Boltzmann
sampling strategy can accurately and, to a large extent, independently
shift the Turner energies of sampled sequences to prescribed targets.
