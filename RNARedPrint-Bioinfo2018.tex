...
\newcommand{\RNAblueprint}{{\tt \bfseries\color{blue}RNABlueprint}}
...
well as sequence-dependent features like \GCb\%.  Furthermore note
...
pair energy model or a stacking energy model $E_{\mathcal{S}}$, whose parameters 
...
To capture the realistic Turner model $E_{\mathcal{T}}$ while keeping the computational cost in check, we exploited the excellent linear fit  of $E_{\mathcal{T}}$  with the fitted stacking model $E_{\mathcal{S}}$ (supp. Fig~\ref{fig:training-cor}). Namely, we observed a structure-specific affine dependency between the Turner and stacking energy models, so that $E_{\mathcal{T}}(S;R) \approx \gamma\cdot E_{\mathcal{S}}(S;R) + \delta$ for any structure $R$ and sequence $S$. We learned the $(\gamma,\delta)$ parameters from a set of sequences generated with homogenous weights $w=e^{\beta}$, tuning only the \GCb\% to a predetermined value.  Finally, we adjusted the targeted energies within our stacking model to $E_{\mathcal{S}}^{\star} = (E_{\mathcal{T}}^{\star}- \delta)/\gamma$ in order to reach, on average, the right targeted energy $E_{\mathcal{T}}^{\star}$ in the Turner model.

Fig.~\ref{fig:energydist} illustrates our above strategy. Fig.~\ref{fig:energydist}A
shows the relative distance between $E_{\mathcal{T}}$  and $E_{\mathcal{S}}$, observed for instances of the \texttt{Modena} benchmark. For each structure, $n=1\,000$ sequences were sampled, while targeting a ${\sf GC}\%$ of $50\%$, and reported the difference between the expected and sampled energies for both the Turner model and its approximation using a stacking model. 
