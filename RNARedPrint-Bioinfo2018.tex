...
%
Recently, the \Software{RNAdesign} approach~\citep{HoenerzuSiederdissen2013}, coupled with powerful local search within \Software{RNAblueprint}~\citep{Hammer2017}, solved the problem of sampling seeds from the uniform distribution for multiple targets. These methods adopt a graph coloring perspective, initially decomposing the graph hierarchically using various decomposition algorithms, and \Def{precomputing} the number of valid sequences within each subgraph. The decomposition is then reinterpreted as a decision tree to perform a \Def{stochastic backtrack}, inspired by Ding and Lawrence~\citep{Ding2003}, where individual assignments are chosen with probabilities derived from the subsolution counts, leading to uniform sampling. The overall complexity of \Software{RNAdesign} grows like $\Theta(4^{\gamma})$, where $\gamma$ is a parameter whose value is bounded only by the length of the designed RNA. 
...
combinatorics, see Bender-Richmond-Williamson~\citep{Bender1983} and
Drmota~\citep{Drmota1997}. In such cases, the expected number of
