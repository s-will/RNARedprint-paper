...
Section~\ref{sec:problem-statement}, are interpreted as functions on the network variables.
...
\item Adding, to each node $x$ in the tree decomposition, the parity variables of connected components that are represented in the node, \emph{i.e.} such that at least one of the positions in the connected component is in $\vars(x)$; \label{step:addparities}
