...
%
  ...
  e.g.~\citep{Turner2009}, and even pseudoknot models,
  e.g. \citep{Andronescu2010}, while bounding the dependencies to
...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    target structures S1 and S2 of subfigure B.  \textbf{(B)} D\&P energy
...
%    ($\delta=0.05, \delta'=0.1, \gamma=1.1, \psi=20, n=1000$)
  ...
  \todo[inline]{Rename S1,S2 to R1 R2. Discuss R2 values. Add legend to subfig A or explain colors}
\end{figure*}
...
\todo[inline]{R1: I am sure your "bipartite" definition is wrong (or your proof is broken), ie. is the bipartition not about V1 and V2, ie there is no edge WITHIN each vertex set but only BETWEEN the sets? Right now, your definition says that the intersection of all possible inter-set-edges (V1xV2) with the present edges E is empty.. But the proof says in line 7-1-35 that any edge "in E involves two nodes $v1 \in V1$ and $v2 \in V2$". So you might correct the bipartite definition to something like "$(V1xV1)\cap E = \emptyset = (V2xV2)\cap E$"  }

To establish that claim, we consider instances $G=(V_1\cup V_2, E)$ that are connected and bipartite ($(V_1\times V_2) \cap E = \varnothing$), noting that hardness over restricted instances implies hardness of the general problem. Moreover note that, as observed in Subsec.~\ref{sec:complexity}, assigning a nucleotide to a position $u\in V$ constrains the parity ($\{\Ab,\Gb\}$ or $\{\Cb,\Ub\}$) of all positions in the connected component of $u$. For this reason, we restrict our attention to the counting of valid designs \emph{up to trivial  symmetry} $(\Ab\leftrightarrow \Cb/\Gb\leftrightarrow \Ub)$, by constraining the positions in $V_{1}$ to $\Ab$ and $\Gb$. Let $\Design{G}$ denote the subset of all designs for $G$ under this constraint, noting that $\NumDesign(G) = 2\cdot|\Design{G}|$.

\todo[inline]{
R1:
7-1-16-18: do you identify the (maximal) independent set for each connected component or do you consider also subsets of the overall independent set to get the "set of all indepedent sets in G"? Please clearify!
%
7-1-29-39: You are using "S" instead of the defined independent set "I". Please correct!
%
7-1-34: The use of "B" comes quite surprisingly since its last occurrence was several pages back. Please add a helping statement like "one has a valid base pair (...) $\in$ B".
}
