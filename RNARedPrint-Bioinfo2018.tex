...
Fig.~\ref{fig:energydist-pk}A shows the good fit between Turner
energies based and energies in the stacking energy model. For the
shown fits we sampled $n=1\,000$ sequences, targeting a ${\sf GC}\%$
of $50\%$.
%
...
\begin{figure*}[t]
  \begin{center}
    {\sf \bfseries A}\ \includegraphicstop[width=0.38\textwidth]{Figs/regression-pk}\hfill
    {\sf \bfseries B}\ \includegraphicstop[width=0.55\textwidth]{Figs/energy_distribution-pk}
  \end{center}
  \caption{%
    Targeting specific energies for pseudoknotted structures using multi-dimensional Boltzmann
    sampling.  (A) Linear fits between the energies in the stacking model to the Turner model for
    sampled sequences; the good match enables more efficient targeting of Turner energies based on
    targeting stacking model energies.
    We show the fits for both target structures S1 and S2 of subfigure B.
    (B) Turner energy distributions for
    two target structures S1 and S2. We show the results of
    targeting the respective free energies $(-20,-10)$, $(-10,-20)$, and $(-15,-15)$
    for the target structures (annotated in red,
    yellow, and blue resepectively)---this demonstrates the
...
\begin{tabular}{@{}>{\bf}l@{\quad}>{\tt}l@{\quad}@{\quad}c@{\quad}c@{\quad}c@{}}
             &   \textbf{\textrm{Dataset}}   & {\bfseries\ourprog{}} & \textbf{Uniform} & \textbf{$\Delta_f$}\\\toprule
  Seeds      & 2str & 21.69 ($\pm$4.37) & 37.74 ($\pm$6.45) & 74\%\\
             & 3str & 18.10 ($\pm$3.98) & 30.49 ($\pm$5.41) & 71\%\\
             & 4str & 20.15 ($\pm$3.83) & 32.32 ($\pm$5.23) & 62\%\\ %\cline{2-5}
             & PK60 & 12.12 ($\pm$3.04) & 19.81 ($\pm$4.24) & 68\%\\
             & PK80 & 13.81 ($\pm$3.63) & 24.05 ($\pm$4.93) & 82\%\\ %\cline{2-5}
             & LE80 & 10.54 ($\pm$2.77) & 17.80 ($\pm$4.17) & 78\%\\
             \midrule

  Optimized  & 2str & 5.42 ($\pm$1.27) & 7.95 ($\pm$1.75) & 35\%\\
             & 3str & 5.10 ($\pm$1.10) & 7.04 ($\pm$1.52) & 31\%\\
             & 4str & 9.17($\pm$1.51) & 13.25 ($\pm$2.13) & 38\%\\ %\cline{2-5}
             & PK60 & 3.75 ($\pm$1.19) & 6.45 ($\pm$1.84) & 125\%\\
             & PK80 & 3.52 ($\pm$1.58) & 7.99 ($\pm$2.35) & 107\%\\ %\cline{2-5}
             & LE80 & 3.59 ($\pm$1.11) & 5.88 ($\pm$1.59) & 215\%\\
             \bottomrule
\end{tabular}\\[1em]
...
\caption{Comparison of the multi-stable design objective function (
  ...
  reports the mean of our improvements over uniform sampling. \TODO{PK datasets ensemble free energy was not available thus approximated with mfe energy.}}
