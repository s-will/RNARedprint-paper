...
\begin{align}
  \label{eq:blueprintobjective}
    \Obj(S)= \quad & \frac{1}{k} \sum_{\ell=1}^{k} (E(S, R_\ell) - \EnsE(S))\notag\\
   & +\frac{1}{2\binom{k}{2}} \sum\limits_{1\leq\ell<j\leq k}|E(S,R_\ell) - E(S,R_j)|.
\end{align}
where $\EnsE(S)$, the \emph{ensemble free energy} of $S$, is computed
by {\tt RNAfold}~\citep{Hofacker1994} in the pseudoknot-free case; for
pseudoknotted targets, $\EnsE(S)$ is approximated by the \emph{minimum
  free energy} of S as estimated by {\tt HotKnots} in the energy model
...
%for RNA design. 
Using \ourprog{}, we generated at least 1\,000 seeds per instance for
...
The results, shown in Fig.~\ref{fig:benchmark-results}, confirm that
...
their MFE, and more similarly stable across targets.  In fact, for
