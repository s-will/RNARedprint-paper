...
\begin{align}
  \label{eq:blueprintobjective}
    \Obj(S)= \quad & \frac{1}{k} \sum_{\ell=1}^{k} (E(S, R_\ell) - \MFE(S))\notag\\
   & +\frac{1}{2\binom{k}{2}} \sum\limits_{1\leq\ell<j\leq k}|E(S,R_\ell) - E(S,R_j)|.
\end{align}
where $\MFE(S)$ is the Minimum Free-Energy of $S$, computed by {\tt RNAfold}~\citep{Hofacker1994} or approximated by {\tt HotKnots}~\citep{Ren2005} for pseudoknotted targets.
Intuitively, the first term of \Obj{} captures how distant from the $\MFE$ the targets are, while the second term penalizes the dispersion of targets. \Obj{} is best (minimized) when all targets simultaneously achieve the $\MFE$ of the sequence.

We considered a benchmark of six sets of target structures described in \citet{Taneda2015}: {\sf 2str}, {\sf 3str}, and {\sf 4str} consist of non-pseudoknotted structures, while {\sf PK60}, {\sf PK80}, and {\sf LE80} contain pseudoknotted structures.
...
%for RNA design. 
Using \ourprog{}, we generated at least 1\,000 seeds per instance for each instance 
of the benchmark. We used target energies that were computed by averaging the energy over samples at
higher weights $e^{\beta}$, and set the targeted \GCb\%
to 60\%.
 We compared the \Obj{} value of 
these sequences against that of seed sequences, uniformly sampled 
using \RNAblueprint{}.
Moreover, for both sets, we used an adaptive greedy
walk~\citep{Hammer2017} to \Def{optimize} the \Obj{} function. At
each step, the local search resamples (uniformly at random) the positions 
of a randomly selected component in the base pair dependency graph, 
accepting the modification only if it results in a gain. The greedy descent
was run for 500 steps in the case of pseudoknot-free datasets {\sf 2str}, {\sf 3str}, and
{\sf 4str}; and 200 steps for the pseudoknotted ones {\sf PK60}, {\sf PK80}, and {\sf LE80}. 


The results, shown in Fig.~\ref{fig:benchmark-results}, confirm 
that Boltzmann-sampled sequences outperform uniform seeds on every dataset, 
leading to average improvements in \Obj{} values ranging from $7.26$ 
({\sf LE80}) to $16.05$  ({\sf 2str}) units. Remarkably, this improvement is
be observed for both of the terms in \Obj{} (see Supp. Mat.~\ref{sec:improvements}). 
This means that \ourprog{} produces sequences whose targets are both substantially closer to their MFE, and more 
similarly stable across targets. 
In fact, for every sequence in our benchmark, consisting of  332 sets of 
target structures in total, we observed better \Obj{} for Boltzmann-weighted sampling 
than for uniform sampling (see Supp. Mat.~\ref{sec:validity}). 
Notably, \ourprog{} remains 
unfazed by the presence of pseudoknots, the main problem being the computation 
of the \Obj{} function, which requires a costly free-energy minimization 
in the presence of pseudoknots~\citep{Sheikh2012}. 
