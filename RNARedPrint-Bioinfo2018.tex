...
RNA energy models like the Turner energy model~\citep{Turner2009} define an energy function that maps a pair of sequence $S$ and structure $R$ to a real value, the \Def{energy}. 
Typically, such energy functions are additively composed from energy contributions of structure elements (like single base pairs or loops). It is instructive, and immediately useful in this context, to first consider the simple \Def{base pair energy model}. This model defines energy contributions $\EbpSym(x,y)$ for base pairs $(x,y)\in\Sigma^2$ that specify the energy function as
$$
\EbpSym(S,R) = \sum_{(i,j)\in R} \EbpSym(S_i,S_j)
$$
for each sequence $S\in\Sigma^n$ and structure $R$ for length $n$.

For multi-target RNA design, we fix the set of target RNA structures
$\R:=\{R_1, \dots, R_k\}$ for sequences of length $n$.
In this work, we are going to generate random sequences that satisfy a number of simple constraints like base pair complementarity for all base pairs in the target structures. Moreover, the generated samples shall (exactly or approximately) satisfy complex properties that we express in terms of features. An example of such a feature is the \GCb\%, where the sequence property would be to have a specific \GCb\%. The further properties we are going to enforce are specific 'target' energies for the single structures $R_1, \dots, R_k$; for this purpose, each such energy is considered as a feature.

Formally, we define a feature $F$ as function on sequences, which is expressed by summing over an associated set of \emph{feature functions} on sequences, such that
$$
\F(S) = \sum_{\text{feature function $f$ of $F$}} f(S).
$$
We emphasize that our approach profits from simple feature functions that depend only on few sequence positions. For example, the $\GCb\%$ feature is expressed by feature functions $f^{GC}_i$ where $f^{GC}_i(S)$ equals 1 if $S_i='G'$ or $S_i='C'$ and 0 otherwise. Similarly the base pair energy for a target structure $R$ is expressed as feature with feature functions $f^R_{ij}(S)=\EbpSym(S_i,S_j),$ such that $F(S)=\EbpSym(S,R)$. In both cases, the feature functions are simple in the way that they depend on the nucleotides at a small (constantly bounded) number of sequence positions. 

\paragraph{Central problem.}
Let us now consider the $k$ energy features $F_1,\dots F_k$ together with the negative $\GCb\%$ as feature $F_0$ to target all of them simultaneously. We base the targeting of the $k+1$ features $F_0,\dots,F_k$ on generating samples from a Boltzmann distribution parametrized by weights $\pi_0,\dots,\pi_k$, one for each feature. Sampling from this distribution requires to compute corresponding partition functions, such that we can draw sequences proportional to their Boltzmann weight:
\begin{equation}
\label{eq:sample-distribution}
\Pr[S;  \pi_0,\dots,\pi_k] \propto \prod_{0\leq \ell\leq k} \pi_\ell^{-F_\ell(S)}.
\end{equation}

At the core of this work, is the efficient (fixed-parameter tractable; FPT) computation of feature-dependent partition functions over sequences.
...
To stress the analogy to, in the RNA context more familiar, Boltzmann distributions over the structure ensemble of a sequence $S$, we emphasize the equivalence
\begin{equation}
...
where the energy $E(S,R_\ell)$ is treated as feature $F_\ell(S) = \EbpSym(S,R_\ell);$ as elaborated later, one could even Turner energies or pseudoknot energies can be expressed as features based on their additive nature. 


\paragraph{Foundations on the complexity of the central problem} 
...
For the base pair energy model, the dependencies are immediately derived from the set of target structures 
$\R$. It induces the
...
First let us define, for single feature functions $f$,
...
Then, the set $\F$ of feature functions (on sequences of length $n$) induces a
  ...
  critically relies on a \Def{tree decomposition} of the dependency
  graph.
...
\section{An fixed-parameter tractable (FPT) algorithm for the
...
\Def{partial sequences}, are words $\val$ in
$(\Sigma\cup\{?\})^n$.

The positions $1\leq i\leq n$, where $\val_i\in\Sigma$,
are called \Def{determined} by $\val$ and form its \Def{domain}.
Note that
$\val$ equivalently represents the set 
of all RNA sequences in $\Sigma^n$ that are identical to $\val$ at the determined positions $i$.
%
...
\item $\F$ is a set of \Def{functions} $f:\Sigma^n\to\real$
\end{itemize}
\end{definition}

%
The network has the natural interpretation of assigning energy to sequences. 
...
$\sum_{f\in\F} \evalfor{f}{S}.$

A further trick lets us handle the feature functions uniformly by `moving' the parameter $\pi_\ell$ of feature $F_\ell$ into each feature function $f$ of this feature, defining a new feature function $f'(S) = 1/\ln(\pi)f(S).$ Then, $exp(-f'(S))=\pi_\ell^{f(S)}.$ While all feature functions belong to a specific feature and therefore need to be weighted accordingly, by assuming that all feature functions are transformed in this way, they can be handled uniformly.

Then, the network energy of $S$ in $\network$ corresponds to the energy in
Eq.~$(\ref{eq:mainproblem}),$ considering the transformation of Eq.~(\ref{eq:feature-energy-transformation}). 
Consequently, the network encodes the partition function $\partfun{\pi_0,\dots,\pi_k}$ of Eq.~(\ref{eq:mainproblem}) and can be used to (now efficiently, as explained in the next section) compute it.
...
i.e.~essentially cluster tree eliminationi (CTE) \citep{Dechter2013}.
...
Let us now consider the computation of the partition function.
...
$\chi(r)=\varnothing$ and $\phi(r)=\varnothing$. If such a root does not exist, it can be easily added to the tree decomposition and connected to one node in each connected component of $T$.

Moreover, we assume that all edges in the tree decompostion are oriented towards this root.
%and 
%define $T_r(u)$ as the induced subtree of
%$u$ (following this orientation). 
...
\todo[inline]{This is now better prepared due to the formulation of the main problem in terms of features with weights. Consequently, the text needs adaptions! Note that the energies E_1,\dots,E_k$ are still features $F_1,\dots,F_k$; there is also a feature $F_0$ for the GC content. }
...
This technique was previously used to control \GCb\%~\citep{Waldispuehl2011,Reinharz2013} and dinucleotide content~\citep{Zhang2013} of sampled RNA sequences; here, we additionally use it to control the free energies $(\TargetE_1,\ldots,\TargetE_k)$ of the single targets. %We use multidimensional Boltzmann sampling to generate sequences having predetermined targeted energies $(\TargetE_1,\ldots,\TargetE_k)$ for the $k$ structures.
...
where $\pmb{\pi}:=(\pi_1\cdots\pi_k)$ is a vector of positive real-valued \Def{weights}, and $\partfun{\pmb{\pi}}$ is the weighted partition function.
...
One then needs to \Def{learn a weights vector} $\pmb{\pi}$ such that, on average, the targeted energies are achieved by a random sequences in the weighted distribution, i.e.\ such that  $\mathbb{E}(E_\ell(S)\mid \pmb{\pi})=\TargetE_\ell$,  $\forall\ell\in[1,k]$.
