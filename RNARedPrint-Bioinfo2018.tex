...
\newcommand{\RNAblueprint}{{\tt \bfseries{} RNA\textcolor{blue!70!black}{Blue}print}}
\newcommand{\ourprog}{{\tt \bfseries{}RNA\textcolor{red!70!black}{Red}Print}}

\newcommand{\Energy}[2]{\textcolor{green!70!black}{E(#2;#1)}}
...
\abstract{\textbf{Motivation:} The design of multi-stable RNA molecules has important applications in biology, medicine, and biotechnology. Synthetic design approaches profit strongly from effective in-silico methods, which can tremendously impact their cost and feasibility. \\
  ...
  specific goals like expected free energies or \GCb\%. Finally,
...
\Software{IncaRNAtion}~\citep{Reinharz2013} controls the \GCb\%
...
\ref{alg:pf} and \ref{alg:sampling}), to sample valid sequences for given
target structures and weights $\pi_1,\dots,\pi_n$.
Moreover, we used multi-dimensional Boltzmann sampling, as described in Section~\ref{sec:multiBoltzmann}, to target specific energies and \GCb\%.  Our tool \ourprog{} evaluates energies according to the base
pair energy model or a stacking energy model $\EnergyStacking$, whose parameters 
were fitted to best approximate Turner energies (Supp. Mat.~\ref{appsec:modelparameters}).
%
...
To capture the realistic Turner model $\EnergyTurner$ while keeping the computational cost in check, we exploited the excellent linear fit  of $\EnergyTurner$  with the fitted stacking model $\EnergyStacking$ (supp. Fig~\ref{fig:training-cor}). Namely, we observed a structure-specific affine dependency between the Turner and stacking energy models, so that $\EnergyTurner(S;R) \approx \gamma\cdot \EnergyStacking(S;R) + \delta$ for any structure $R$ and sequence $S$. We learned the $(\gamma,\delta)$ parameters from a set of sequences generated with homogenous weights $w=e^{\beta}$, tuning only the \GCb\% to a predetermined value.  Finally, we adjusted the targeted energies within our stacking model to $\EnergyStacking^{\star} = (\EnergyTurner^{\star}- \delta)/\gamma$ in order to reach, on average, the right targeted energy $\EnergyTurner^{\star}$ in the Turner model.

Fig.~\ref{fig:energydist} illustrates our above strategy. Fig.~\ref{fig:energydist}A
shows the relative distance between $\EnergyTurner$  and $\EnergyStacking$, observed for instances of the \texttt{Modena} benchmark. For each structure, $n=1\,000$ sequences were sampled, while targeting a ${\sf GC}\%$ of $50\%$, and reported the difference between the expected and sampled energies for both the Turner model and its approximation using a stacking model. 
Fig.~\ref{fig:energydist}B shows the Turner energy, for a single instance of the 3-structures \texttt{Modena}  benchmark, using different weights benchmark. These results demonstrate that our multidimensionnal Boltzmann sampling strategy can accurately and, to a large extent, independently shift the Turner energies of sampled sequences to prescribed targets.  
...
\begin{figure*}[t]
  \begin{center}
    {\sf \bfseries A}\includegraphicstop[width=0.32\textwidth]{Figs/offset}\hspace{.5cm}
    {\sf \bfseries B}\includegraphicstop[width=0.57\textwidth]{Figs/energy_distribution}
  \end{center}
  \caption{%
  	Targeting specific energies using multi-dimensional Boltzmann sampling. 
  	(A) Accuracy of targeting over all target structures of the \Software{Modena} benchmark instances \texttt{RNAtabupath} (2str), \texttt{3str} and \texttt{4str}. Shown is the relative deviation of targeted and achieved energies in the simple stacking model and the Turner model.
  	(B) Turner energy distributions for three target structures (right) while targeting $(-40,-40,-20)$ and $(-20,-20,-20)$ free energy vectors. For comparison, we show the distribution of uniform and Boltzmann samples; respectively associated with homogenous weights $1$ (uniform) and $e^\beta$ (Boltzmann). 
...
uniformly sampled using \RNAblueprint{}~\citep{Hammer2017}
...
constraints, like the \GCb\%, while controlling the energy of
