...
strategy. Fig.~\ref{fig:energydist}A
illustrates our capability to target Turner energies based on the
stacking energy model. It shows the deviation between
...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    (2str), \texttt{3str} and \texttt{4str}. 
    For the stacking and the Turner energy model, the relative offsets between the targeted and 
    achieved energies were determined. We show the deviations of these offsets between both models.
    (B) Turner energy distributions for
...
We now establish the complexity of $\NumDesign(G)$, the problem of computing the number of valid sequences for a given compatibility graph $G=(V,E)$. Note that $\NumDesign(G)$ is equivalent to computing the partition function problem in a simple $(0/\infty)$-valued base pair model, with $\beta\neq 1$. As previously noted~\citep{Flamm2001}, a set of target structures admits a valid design iff its compatibility graph is bipartite, which can be tested in linear time.
