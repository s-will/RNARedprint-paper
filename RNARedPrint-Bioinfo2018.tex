...
\todo[inline]{Plan: make the RNA design network more specific, i.e. define the set of feature functions to containt the GC and energy features (in the specific form with internalized weights).}
...
$(\Sigma\cup\{?\})^n$, where '?' is the dummy-value of undetermined positions, which allows representing only partially determined sequences.
Consequently, the positions $1\leq i\leq n$, where $\val_i\in\Sigma$,
are called \Def{determined} by $\val$.
