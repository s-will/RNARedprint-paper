...
\Answer{We structured and rephrased the Results section more clearly and adapted the introduction accordingly. In Results, we introduce single subsections for the following sub-results to reflect our main results in a logical order:
  \begin{itemize}
    \item The full-featured RNA energy model (Turner) can be well approximated by the much more efficiently tractable stacking model; in this model, controlled Boltzmann sampling for typical multi-target design instances requires little time (corresponding to small tree-width);
