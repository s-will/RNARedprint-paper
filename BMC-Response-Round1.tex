...
\AnswerTODO[Yann]{We structured the results section more clearly and adapted the introduction accordingly. In Results, we introduce single subsections for the following sub-results to reflect our main results in a logical order:
  \begin{itemize}
    ...
    \item Controlled Boltzmann sampling for typical multi-target design instances requires little time (corresponding to small tree-width).
...
\Answer{We agree that running times are important for assessing the method; consequently, we included details timing information in the revision. As we show now explicitly, the running times directly depend on the already reported tree-widths. The run-times are generally modest for the relevant design instances.} 
...
\AnswerTODO[Stefan]{
  We do now report and discuss the run-times of such methods for generating samples. Since MODENA does not allow to measure separate times for sequence sampling, we estimated those times and report our estimates. It should be emphasized that MODENA produces a very different kind of sampling compared to RNARedPrint (and as well RNABluePrint), such one should be careful to compare only run-times. While RNARedPrint produces a very specific distribution of samples, solving a hard positive design problem, MODENA  
