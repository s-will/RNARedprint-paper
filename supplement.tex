...
that---briefly put---negative design benefits from positive design. Here, more
concretely, we only \emph{demonstrate} this effect (for small examples), where
...
For a double-stem structure, we produce a uniform and a weighted Boltzmann
...
%
Consequently, Figure~\ref{fig:negforpos} ultimately illustrates the dependency
between $Pr(S|R)$ and $Pr(R|S)$ for uniform samples and low energy samples
(where the latter can be efficiently generated due to our Boltzmann sampling
strategy). 
...
\caption{Comparision of (A) Uniform sampling to (B) Boltzmann sampling with
weight $\pi=500$. For given structure $R$, we plot the energies $E(S,R)$ of
