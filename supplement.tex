...
induction argument: in this way, the message computation over one node
...
the partition function, and the restricted partition function coincides 
with the initial one.



Remarkably, this restriction drastically improves the time
complexity. While the original algorithm enumerates all partial
sequences $\chi(v):=\substitute{\val}{\val'}$ such that
$\val\in\partseqs(\separator{u}{v})$ and
$\val'\in\partseqs(\difference{u}{v})$, the modified algorithm
only considers cc-valid sequences for $\chi(v)$ the nodes of $v$.
Namely, 
