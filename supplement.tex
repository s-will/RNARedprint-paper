...
\emph{all} possible combinations of nucleotides for the positions in
the separator set, we observe here that this is \emph{not} required for computing all relevant
partition functions. In particular, the algorithm can be restricted to
consider only \emph{valid} combinations, which satisfy the (hard)
constraints on the nucleotides.

RNA design introduces constraints due to the requirement of canonical
(aka complemenentary) base pairing, which can be exploited in a
particularly simple and effective way.  As already noted by
\citet{Flamm2001}, the base pair complementarity induces a
...
nodes in the partition are restricted to values $\{\Ab,\Gb\}$ and
$\{\Cb,\Ub\}$ respectively; we call a partitial sequence
\emph{cc-valid}, iff its determined positions satisfy this separation
...
One can now modify Alg.~\ref{alg:pf}, on tree decomposition
$(T,\chi)$, such that the message values
$\evalfor{\Message{u}{v}}{\val}$ are computed only for cc-valid
partial sequences $\val\in\partseqs(\separator{u}{v})$. Moreover, the
loop over $\val'\in\partseqs(\difference{u}{v})$ is restricted, such
that $\substitute{\val}{\val'}$ are cc-valid. (The analogous change is
made to the sampling algorithm Alg.~\ref{alg:sampling}.)

The correctness of the modified algorithm still follows from an
induction argument: in this way, the message computation of one node
evaluates messages from its children only at cc-valid partial
sequences. The result of this computation is a message, which is
defined at all cc-valid partial sequences.

Remarkably, this strongly improves the algorithm's time
complexity. Where the original algorithm enumerates all partial
sequences $\substitute{\val}{\val'}$, where
$\val\in\partseqs(\separator{u}{v})$ and
$\val'\in\partseqs(\difference{u}{v})$, the modified algorithm
enumerates in two stages:
\begin{enumerate}
\item for each connected component of the base pair dependency graph
  that involves any position in
  $\separator{u}{v}\cup\difference{u}{v} = \chi(v)$, it first
  enumerates all possibilities how the positions from the partition
  sets are assigned to either $\{\Ab,\Gb\}$ or $\{\Cb,\Ub\}$
\item it enumerates the possible partial sequences from the (according
  to the decisions of the first stage) restricted domains of the
  position in $\chi(v)$.
\end{enumerate}
...
$\hat w := \max_{v\in T} c(v)+|\chi(v)|$. Notably, $\hat w$ can not
exceed $2(w+1)$ such that the modified algorithm does not increase the
original complexity.
%
As useful corollary, one obtains the (looser) complexity
bound $$O(nk2^{w+1}2^{cc})$$ in terms of treewidth $w$ and connected
components $cc$.
...
constraints, in a complete and general way, likely causes significant
overhead in an implementation and does not straightforwardly improve
