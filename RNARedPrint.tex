...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    targeting stacking model energies. \textbf{(B)} \revised{Resulting D\&P energy distributions for the two target structures R1 and R2 when aiming for the respective free energies $(-30,-20)$,
...
%for RNA design.
Using \revised{multi-dimensional Boltzmann sampling based on} \ourprog{}, for
each instance of the benchmark, we generated at least 1\,000 seed sequences with similar energies for all target structures. 
...
\revised{As described in Section~\ref{sec:multiBoltzmann}, to find sequences fulfilling our energy and \GCb\% criteria, we iteratively sampled 10\,000 sequences and kept admissible ones while optimizing the weights. In practice, this sampling operation performs quite fast (see \ref{appsec:run-times}) and only little iterations were needed for most of the benchmark inputs as reported in  \ref{appsec:effectivity-seed-generation}.}
We compared the
