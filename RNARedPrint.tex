...
Let us consider $m$ (secondary) structures $R_1,\dots, R_m$, each abstracted as a set of base pairs, and \revised{$k\geq m$} features $F_1,\dots, F_k$\revised{, typically representing the energies of the structures and additional sequence properties,} associated with weights $\pi_1,\dots,\pi_k$. Our goal is to sample sequences \revised{$S$ (which satisfy the base pairing rules for all strucutures) from the Boltzmann distribution defined by} 
\begin{equation}
...
%
  ...
  \end{equation}
  for specfic weights $\pi_1,\dots,\pi_k$.
...
$$
\revised{Here, since $\dep(f)=\{x_1,\ldots,x_p\}$, $\left\{\substack{x_1\mapsto S_{x_1}\\\cdots\\x_p\mapsto S_{x_p}}\right\}$ denotes the assignment, that assigns to all positions $x_k$  in $\dep(f)$ the respective nucleotide $S_{x_k}$ ($1\leq k\leq p$).} 

The $\GCb\%$ can be simply expressed using $n$ contributions $f^{\GCb}_i$, \revised{each dependencing only on position $i\in[1,n]$, i.e.~$\dep(f^{\GCb}_i)=\{i\}$}, such that 
