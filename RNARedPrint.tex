...
\todo{$x_1$ position(?) not defined!\\$S_{x_1}$ nucleotide at position(?) not defined!\\$p$ number of dependent positions(?) from $x_p$ not defined!\\
Yann: We just defined the dependencies as positions in the sequence, so $x_i$ should be fine from the context. $p$ is purely declarative (it differs for each $f$), so it should be ok, but since it is unused, we can probably drop it...
}
...
$$
F(S) =\!\!\!\!\!\!\!\sum_{\substack{\text{$f$ contribution of $F$,}\\\text{ $\dep(f)=\{x_1,x_2,\ldots\}$}}}\!\!\!\!\!\! f\left(\left\{\substack{x_1\mapsto S_{x_1}\\x_p\mapsto S_{x_2}\\\cdots}\right\}\right).
$$
where any $x_i \in [1,n]$ represents a position in $S$, and $S_x$ represents the letter found at position $x$ in $S$.
...
\todo{Why here $i$ and not $x_i$??\\Yann: Because $x_1$, $x_2$... was just a way to talk about the first, second... argument of a function having an arbitrary number of args. Here, there is just one, so no need for subscripts.}
...
However, while in principle feasible, sampling in a Boltzmann distribution directly using the Turner energy model may induce extreme computational demands, with tree-widths scaling at least as large as the number of nucleotides in the largest loop. Fortunately, we found that intricacy of the Turner energy model can be circumvented with minimal loss of precision by using a simpler stacking energy model. As can be shown in Figure~\ref{fig:correlation}, a simple stacking energy model, further described in Supp. Mat.~\ref{appsec:modelparameters} can be used to approximate the Turner energy model (correlation coefficient $R=0.99$), while greatly reducing the tree width, and thus the computational requirements of the whole method even for complex instances.
...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    model by Dirks and Pierce (D\&P) for \revised{initially} sampled sequences and both target structures R1 and R2 (shown in subfigure B). The good
    match \revised{(respective $R^2$ values of $0.846$ and $0.841$)} enables more efficient targeting of Turner energies based on
    targeting stacking model energies. \textbf{(B)} Resulting D\&P energy distributions for the two target structures R1 and R2 when aiming for the respective free energies $(-30,-20)$,
    $(-30,-30)$, $(-25,-25)$, and $(-20,-30)$. These demonstrate the effectivity of our adaptive
    multi-dimensional Boltzmann sampling procedure, especially by comparing the distributions to those of uniform and Boltzmann sampled sequences, with homogenous weights $1$ and
    $e^\beta$, respectively.
...
improvement is observed for both of the terms in \Obj{} (see
