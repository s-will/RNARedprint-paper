...
The main computational problem addressed in this work is to generate sequences over the alphabet $\Sigma=\{\Ab,\Cb,\Gb,\Ub\}$ that achieving prescribed free-energies and $\GCb\%$.
Despite their apparent diversity, such criteria can be homogeneously abstracted as constraints on the value of \Def{features}, each defined as sums over atomic feature contributions depending on subsets of sequence positions.
 
\paragraph{Central computational problem.}
Let us consider $k$ features $F_1,\dots F_k$ to target all of them simultaneously. We base our strategy for targeting the $k$ features $F_1,\dots,F_k$ on generating samples from a Boltzmann distribution parameterized by weights $\pi_1,\dots,\pi_k$, one for each feature. These weights essentially allows us to shift the sample distributions towards specific targets.
...
Formally, we define a \Def{feature $F$} as function on sequences, whose value is obtained by summing over an associated set of \Def{atomic feature contributions} $f_1, f_2\ldots$, briefly called \emph{contributions}, that each are associated with a restricted set of positions, such that
$$
F(S) =\!\!\!\!\sum_{\substack{\text{contribution $f_i$ of $F$}\\ \text{on positions $x_1,\ldots,x_p$}}}\!\!\!\! f_i(\{x_1\mapsto S_{x_1},\ldots,x_p\mapsto S_{x_p}\}).
$$

As simple example, the $\GCb\%$ feature is expressed by $n$ contributions $f^{\GC}_i$ where $f^{\GC}_i(\{i\mapsto S_i\})$ equals $-1$ if $S_i=\Gb$ or $\Cb$, and $0$ otherwise. 
...
In a simple \Def{base pair energy model}, the free-energy of a target structure $R$ is expressed as feature with contributions $f^R_{ij}(\{i\mapsto x, j\mapsto y\})=\EbpSym(x,y)$ for each $(i,j)\in R,$ such that $F^R(S)=\EbpSym(S,R)$. 

Note that for \GCb\% as well as for the base pair energy features, the contributions depend on the nucleotides at only a small, constantly bounded number of sequence positions (one and two positions, respectively). This is important for the feasibility of our approach, which is designed to profit from contributions that each depend on only few sequence positions. 
...
More generally, we consider dependencies induced by the complete set of contributions $\F := \bigcup_{F}\{f\mid f\text{ contribution of } F\}$.
Recall that each contribution, even if `formally' evaluated for entire sequences, depends only on a subset of all sequence positions, which we call the \Def{dependencies of $f$} and write as $\dep(f)$;
thus, $\dep(f)$ is the minimal subset of $\{1,\dots,n\}$ s.t.~$\forall S,S'\in\Sigma^n: f(S)=f(S') \text{ only if } (\forall i\in\dep(f): S_i=S_i')$.
  
Then, the set $\F$ of contributions induces a
...
Our algorithms iteratively merge partial solutions. To capture this we introduce 
\Def{Partial RNA sequences} as words $\val$ in
$(\Sigma\cup\{?\})^n$, where the symbol '?' indicate unassigned positions; consequently, positions such that  $\val_i\in\Sigma$,
...
Since each contribution $f\in\F$
...
For the algorithms, it is convenient to encode the weight $\pi$ of each feature in its contributions, such that all contributions can be handled uniformly. For this purpose, we define the \Def{transformed contribution} 
...
\begin{itemize}
\item the transformed contributions $\weighttransform{f^{\GCb}_i}{\pi_0}$ for the ~$\GCb\%$ feature ($i=1,\dots,n$)
\item the transformed contributions $\weighttransform{f^{R_\ell}_{ij}}{\pi_\ell}$for each structure $R_\ell\in\R$ and $(i,j)\in R_\ell.$
\end{itemize}

By these definitions, the network encodes the partition function $\partfun{\pi_0,\dots,\pi_k}$ of Eq.~(\ref{eq:mainproblem}); also confer Eq.~(\ref{eq:feature-energy-transformation}). Subsequently, we show how to efficiently compute this partition function based on the network.

\subsubsection*{Partition function and  stochastic backtrack algorithms}\label{sec:PF}
...
\textbf{Move the approximation of more complex models from "Results" to this subsection}
