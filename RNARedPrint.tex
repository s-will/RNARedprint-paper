...
\subsection*{Dependency (hyper)graph, tree decomposition and width}
...
\revised{In order to compute the partition function of Eq.~\ref{eq:mainproblem}, and thus sample in a well-defined distribution, one needs to abstract dependencies beyond pairs of positions. To that purpose, we consider the notion of \Def{hypergraph}, which consists of vertices (here, the sequence positions) connected by \Def{hyperedges} defined as sets of vertices. Undirected graphs can be modeled as hypergraphs whose hyperedges are pairs of vertices, but hypergraphs also allow to express complex dependencies.

Here, we consider the complete set of contributions $$\F := \bigcup_{\ell}\{f\mid f\text{ contribution of } F_\ell\}.$$
%
%Let us denote the \emph{dependencies of a contribution $f$} as \dep{f}.
%
The set $\F$ of contributions induces a
\Def{dependency (hyper)graph} $G_\F$ on sequence positions by interpreting the dependencies as hyperedges, leading to an hypergraph 
$G_\F=(V, H)$ having vertices $V=\{1,\dots,n\}$ and hyperedges $H = \{\dep(f)\mid f\in \F\})$.

Irrespectively of the chosen energy model, to capture the requirements of canonical base pairing for each structure,
we also define the \Def{base pair dependency graph} $G_{\R} = (V,E)$ having vertices $V=\{1,\dots,n\}$ 
and edges $E=\bigcup_{\ell\in[1,k]} R_\ell$. Any edge between two nodes $i$ and $j$ materializes the existence of a base pair between positions $i$ and $j$ in at least one of the input structures, and induce a direct dependency between the nucleotide assignments for those positions.
}

Let us now remind the \Def{tree decomposition} of an (hyper)graph $G_\F$, a fundamental ingredient of our algorithms. 
...
The \Def{width} of a tree decomposition $(T,\chi)$ is 
$$\min_{u\in T} |\chi(u)| - 1.$$ The \Def{treewidth}
of an (hyper)graph $G$ is the smallest width of a tree decomposition of $G$.
\end{definition}
