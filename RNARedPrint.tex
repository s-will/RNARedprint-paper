...
%
  ...
  design methods: the generation of sequences \revised{that fold well into multiple target structures.} For this task, we present the fixed-parameter tractable (FPT) approach \ourprog{}.
  % 
  \revised{Based on} constraint networks, \ourprog{} supports generic
...
%
  %Securing the theoretical foundations of our method
  Moreover, we establish the $\#${\sf P}-hardness of \revised{counting to theoretically justify the development of an FPT approach}. 
%
  Finally, we study general properties of our approach empirically and generate
  biologically relevant multi-target Boltzmann-weighted designs for a
  common design benchmark.
  We demonstrate significant improvements over the previously best multi-target sampling strategy, due to generating seed sequences with \ourprog{}.\\
%
...
works well by performing sampling based on \revised{a simplified RNA energy model, which induces much lower treewidth than the Turner model}. This result is \revised{essential for the applicability of our method, since it allows to combine high efficiency (by keeping the tree width low) with sufficient accuracy to precisely target realistic Turner energies.} 
...
Here, such criteria are modeled} as constraints on the values of \Def{features}, functions of the sequence that are expressed as sums over real-valued contributions. Such contributions typically depend on the nucleotides at specific sequence positions.
...
On this basis, we can finely calibrate the weights, to increase the probability that sampled sequences meet prescribed target values for each feature. Together with a final rejection step this results in an effective procedure for generating highly specific sequences.}
...
\todo[inline]{Check use of the term stacking model; the term is not introduced here yet!}
...
relate more than two positions, its dependencies cannot be similarily represented by a
graph. Instead, this requires a structure known as \Def{hypergraph}, which
consists of vertices (here, the sequence positions) connected by
\Def{hyperedges} are sets of vertices---in generalization of graphs, where edges are pairs of vertices.
%Undirected graphs can be
%modeled as hypergraphs whose hyperedges are pairs of vertices, but
%hypergraphs also allow to express complex dependencies.

The set $\F$ of contributions induces the \Def{dependency
  (hyper)graph} $G_\F=(V, H)$ on sequence positions $V=\{1,\dots,n\}$
by interpreting the dependencies as hyperedges, i.e. $H =
\{\dep(f)\mid f\in \F\})$.

Let us now define the \Def{tree decomposition} of the graph $G_\F$, a fundamental ingredient of our algorithms, which also determines their efficiency (most importantly, via its property called treewidth).
}
...
$$\min_{u\in T} |\chi(u)| - 1.$$ 
The \Def{treewidth}
of a graph $G$ is the smallest treewidth of any tree decomposition of $G$.
\todo[inline]{Check: do we really need to define treewidth of a graph?}
\end{definition}
...
Figure 2 -- Distribution plot -- Right (Turner is even untractable to compute the tree width itself in benchmark instances)
