...
%
  ...
  design methods: the sampling of sequences for multi-target RNA design. For this task, we present the efficient (fixed-parameter tractable) approach \ourprog{}.
  % 
  Modeling
...
proxy for rational design approaches, ultimately targeting catalytic or regulatory functions~\citep{Zhang2013}.
...
meaningful, representative benchmark.
...
Formally, we define a \Def{feature $F$} as function on sequences, whose value is obtained by summing over an associated set of \Def{atomic feature contributions} $f_1, f_2\ldots$, briefly called \emph{contributions}, that each are associated with a restricted set of positions, its \emph{dependencies $\dep(f)$}, such that
$$
F(S) =\!\!\!\!\!\!\sum_{\substack{\text{contribution $f_i$ of $F$}\\\text{with $\dep(f)=\{x_1,\ldots,x_p\}$}}}\!\!\!\!\!\! f_i(\{x_1\mapsto S_{x_1},\ldots,x_p\mapsto S_{x_p}\}).
$$

As simple example, the $\GCb\%$ feature is expressed by $n$ contributions $f^{\GCb}_i$ where $f^{\GCb}_i(\{i\mapsto S_i\})$ equals $-1$ if $S_i=\Gb$ or $\Cb$, and $0$ otherwise. 
...
To formalize our algorithms, which iteratively merges evaluations of partial solutions, we extend the idea of atomic feature contributions, which are evaluated at sets of the form $\{x_1\mapsto v_1, \dots, x_p\mapsto v_d\}$. Let us call the latter object \Def{partial sequence}; they are handy to specify partial knowledge on the entire sequence at some point of the algorithm. Easily, we can extend the definition of contributions $f$ to sets $\{x_1\mapsto v_1, \dots, x_p\mapsto v_d\}$, where $\{x_1\dots x_d\}$ is any super-set of $\dep(f)$ by ignoring the superfluous assignments $x\mapsto v$, where $x\not\in\dep(f)$. 
...
For algorithmic treatment, it is convenient to encode the weight $\pi$ of each feature in its contributions, such that all contributions can be handled uniformly. For this purpose, we define the \Def{transformed contribution} 
...
on a tree decomposition of the $\F$'s dependency graph.
...
Our algorithms are formulated to process a \Def{cluster tree}, which is a tuple $(T,\chi,\phi)$, where
