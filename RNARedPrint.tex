...
\parHead{Motivation.}
%Better understanding of the true complexity bounds is highly relevant for the application of such methods, in particular for applications in more expressive settings (e.g. Boltzmann sampling as discussed in this work or sampling satisfying multiple additional constraints and properties).

%All such extensions are still limited by the complexity of the multi-target uniform sampling problem, where counting of valid designs is the dominating step. 
...
\end{figure*}
%
In particular, we ask in this paper: \emph{How to sample, in a general way, from a Boltzmann distribution (which is a natural generalization of uniform sampling) based on expressive energy models? How to enforce additional constraints, such as \GCb\%, complex sequence constraints, or the energy of individual structures?}

For addressing these questions,
we introduce a generic framework (illustrated in~Fig.~\ref{fig:workflow}) enabling efficient Boltzmann-weighted sampling over RNA sequences with multiple target structures. As main algorithmic contribution, we devise a dynamic programming algorithm, based on a tree decomposition, to compute partition functions. This serves as precomputation for a second algorithm, which samples sequences from the Boltzmann distribution%
% (Subsection~\ref{sec:PF})
. We show that both algorithms are time and space-efficient for a fixed 
 property of the tree decomposition, called the treewidth.
This parameter, has an exponential impact on the complexity, but is low for typical instances; in short, our algorithms are fixed-parameter tractable (FPT).

To theoretically justify the exponential dependency on the treewidth, we show that the underlying counting problem is \#{\sf P}-hard; this implies that even the counting problem cannot be solved efficiently (i.e.~in polynomial time) unless ${\sf P}={\sf NP}$. This complexity result relies on a surprising bijection between valid sequences and independent sets of a bipartite graph; the latter being the object of recent breakthroughs in approximate counting complexity~\citep{Bulatov2013,Cai2016}.

% Subsection~\ref{sec:complexity}
%
In practice, due to the systematic construction of our method, which is based on tree decomposition, we can strongly limit the treewidth by using state-of-the-art tree decomposition algorithms.
%
Moreover, we support arbitrary multi-ary constraints and thus
...
Finally, we describe an \Def{adaptive
...
works well by performing sampling based on less complex RNA energy models, which keeps the treewidth low. In practice, this allows to combine high efficiency with high accuracy and moreover leaves room for extensions of the method.
...
The flexibility of our framework supports the advanced sampling technique "multidimensional Boltzmann sampling"~\citep{Bodini2010}, which allows (probabilistically) enforcing additional, complex properties of the samples.
...
\ref{alg:pf} and \ref{alg:sampling}) was implemented, using a combination of {\tt C++} and {\tt Python}, in a software called \ourprog{}, available at: 
...
We implemented the core algorithms in C++, resulting in the tool
C++-tool \ourprog{}, which takes a set of target structures, as well as
weights for each energy feature and \GCb\%; then,
it generates a sample (of prescribed size) from the corresponding
Boltzmann distribution. For \ourprog{}, we implemented the base pair
energy model and the stacking energy model.
Moreover, we provide a Python wrapper script for the C++-tool
\ourprog{} to perform targeting, based on multi-dimensional Boltzmann
sampling.
For given secondary structures, together with prescribed target energies
and target \GCb\%, this script generates
a series of sequences that satisfy the target values for the energy and
\GCb\% features within configurable tolerances.
Notably, these target energies are actual free energies in the realistic
Turner energy model, which are targeted by efficiently sampling in the
...
{Stefan: \em Mention RNAsketch to perspectives for more complex design scenario + need for generic constraints-based framework for flexibility ($\to$ generic constraint framework).}

{Yann: \em Connection with statistics (design as perfect random sequences for null-model), mention Rivas/Eddy lncRNAs papers...}
